...
void home_page(uint32_t arg=0) {
	...
	settings_button[0]->setAction(handleClick_Settings, 0); // go to settings page
	...
	uint16_t level_to_pixel = (uint16_t)((battery_level/100)*rect_width); // conversion to an integer
	...
	tft.drawString(200, 0, battery_level_str);
   	...
   	tft.drawVerticalLine(x1_rect+1, y0_rect+4, 2, WHITE);
   	if (battery_level > 74) {
   		tft.fillRectangle(x0_rect+1 , y0_rect+1 , x0_rect+1 + level_to_pixel, y1_rect-1, GREEN);
   	}
   	else if ((20 < battery_level)&&( battery_level < 75)) {
   		tft.fillRectangle(x0_rect+1 , y0_rect+1 , x0_rect+1 + level_to_pixel, y1_rect-1, YELLOW);
...
/*void handleClick_Battery(uint32_t arg=0) {
	...
	uint16_t rect_width = y1_rect - y0_rect;

	uint16_t level_to_pixel = (uint16_t)(((100-battery_level)/100)*rect_width); // conversion to an integer
	char battery_level_str[6];
	const char percent[] = "%";
...
void handleClick_Settings(uint32_t arg) {
	...
	settings_buttons[0]->setAction(home_page, 0); // back to the home page
